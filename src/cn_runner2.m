...
function [is_blackout,GC_size,MW_lost] = cn_runner2(exp_id, config_json, exps_to_run_file, starting_run)
% Inputs:
...
coupled_nodes_filename = strcat('/tmp/', 'coupled_nodes_ieee30coup_', pid, '.csv');
...
delete(strcat('/tmp/','coupled_node_ieee30_',exp_id,'.csv'));
...
comm_file_name = [opt.comm.relpath,'data/coupled-nodes/coupled_node_ieee30_',sprintf('%s',exp_id),'.csv.bz2']; % %s so exp_id gets converted to a string
...
p_column = 1
if p_column > numOutagesInCouplingFile || p_column == 0
...
numOutagesInFile = 27; % TODO get this in opt.comm
inverseStepSize = 100; % TODO get this in opt.comm
minOutagePoint = 0.0; % TODO get this in opt.comm

% split_path = strsplit(opt.comm.comm_model,filesep);
% relpath = split_path(1:length(split_path)-2);
% relpath = sprintf('%s/', relpath{:});    % should give "coupled-networks/" root folder

outage_file_name = [opt.comm.relpath,'data/node-removals/bus_outage_ieee30_',sprintf('%s',exp_id),'.csv.bz2']; % %s so exp_id gets converted to a string
...
% p_column = 1
% print "p_column " + str(p_column) + " from file: " + file_name
if p_column > numOutagesInFile || p_column == 0
    randomRemovalFraction=1-p_point;  % Fraction of nodes to remove
